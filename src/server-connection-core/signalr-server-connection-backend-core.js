...
angular
   ...
   */
  .factory("ServerConnectionBackendCore", [
    function () {
      function ServerConnectionBackendCore() {
        var disconnectFns = {};
        this.onDisconnect = function (name, fn) {
          if (!disconnectFns[name]) {
            disconnectFns[name] = [];
          }
          disconnectFns[name].push(fn);
        };
        ...
        this.getConnection = function (hubName) {
          ...
          hubConnectionBuilder.withUrl("hubs/" + hubName, {
        ...
        this.start = function (hubName) {
          try {
            var con = this.getConnection(hubName);
            if(con.state != signalR.HubConnectionState.Disconnected){
              console.log("Already connected to " + hubName);
              return new Promise(function(){});
            }
            var result = con.start();
        ...
        this.stop = function (hubName) {
          ...
          if(con.state != signalR.HubConnectionState.Connected){
            console.log("Already disconneted from " + hubName);
            return new Promise(function(){});
          }
          return con.stop();
        };
        ...
        this.getMethodNames = function (hubName) {
          return this.getConnection(hubName)
            .invoke("functionList")
